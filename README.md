...
如果你经常折腾代理、跑号服务器、API 连通性，v6.3 这一版就很适合放在自己的机器上跑。它不是那种“我在别的机器上测通了，所以你那里也一定能用”的玄学榜单，而是直接回答一个更实用的问题：**你这台服务器能不能连上这个代理，并且通过它访问目标服务？**

🚀 项目地址：[henryz78/proxy-checker](https://github.com/henryz78/proxy-checker)
...
- 页面版本号更新到 `v6.3`，README 和 PRODUCT 同步更新发布说明。
- **高精度 IP 风控检测 (迁移自 gatevpn)**：后端 `proxy_check.py` 升级为并发多源 IP 风控检测系统，同时并发查询 `ip-api.com`、`ipwho.is`、`proxycheck.io` 以及 4 大垃圾源/防垃圾黑名单 DNSBL（Spamhaus、Spamcop、Sorbs、S5h）。
- **风控画像与欺诈值**：计算输出 IP 风控欺诈分值（0-100）、风险评级（高/中/低/干净）、风险标记（如 `tor_exit`, `vpn_flag`, `proxy_flag` 等）及黑名单命中次数。
- **5 级 IP 类型精确划分**：IP 类型由原来的“机房/住宅”二级细化为“住宅 IP、机房 IP、移动网、代理 IP、Tor 出口”五级。
- **前端风控详情面板**：有效/失效列表中的代理卡片展开后，会直观展示风控值、风险等级（不同风险用红黄蓝绿高亮区分）和黑名单详情。
- **仓库多维过滤**：我的仓库（My Repository）面板增加了对移动网、代理 IP、Tor 出口的多维过滤按键，并对等级筛选 UI 进行了整合。
...
```bash
git clone https://github.com/henryz78/proxy-checker.git
cd proxy-checker

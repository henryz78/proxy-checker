...
- 页面、README 和 PRODUCT 版本号同步更新至 `v6.3`。
...
- **大任务可靠性与防卡死优化**：修复了大批量自动任务可能卡在最后少量代理、长期无法结束的问题。
- **Worker 队列并发控制**：批量检测从“一次性创建全部异步任务”改为固定数量 worker 队列，面对上万条代理时减少任务堆积和内存压力。
- **单个代理独立硬超时**：为单个代理增加独立硬超时，遇到不按预期返回的网络请求时，会记录为 F 级失败并继续检测，不再拖死整轮任务。
- **任务终止流程加速**：收到停止信号后会立刻取消尚未完成 of worker，停止操作不再长期停留在 `stopping` 状态。
- **检测进度辅助诊断**：自动任务摘要新增 `last_progress_at`，便于判断任务最后一次产生结果的时间，排查卡顿更直接。
- **云端恢复与安全防串**：修复云端仓库已经清空时，“恢复云端数据”仍保留浏览器本地旧代理的 bug。从空云端恢复时会明确清空本地仓库并标记，且云端仓库读取不再回退到公共 `default` 仓库，避免不同用户 token 之间出现历史数据串仓。
